...
func TestSignatoryEditV3(t *testing.T) {
	req := request.SignatoryEditV3Request{}
	req.SignatoryId = "3298942387094663511"
	req.TenantName = "宋一"
	req.SignValidateWay = "PIN"
	receiver := model.User{}
	receiver.Contact = "10036350001"
	receiver.ContactType = "MOBILE"
	req.Receiver = &receiver
	operator := model.User{}
	operator.Contact = "10000000011"
	operator.ContactType = "MOBILE"
...
func TestContractViewPage(t *testing.T) {
	...
	downloadButton := true
	printButton := false
	req.DownloadButton = &downloadButton
	req.PrintButton = &printButton
	//req.Lang = "en"
	response, err := sdkClient.Service(req)
	if err != nil {
		fmt.Println("request failed,", err.Error())
...
func TestContractModify(t *testing.T) {
	req := request.ContractModifyRequest{}
	req.Id = "3331608603449639726"
	req.Subject = "修改后的主题"
	response, err := sdkClient.Service(req)
	if err != nil {
		fmt.Println("request failed,", err.Error())
...
func TestSignatoryOperatorsEdit(t *testing.T) {
	req := request.SignatoryOperatorsEditRequest{}
	req.ContractId = "3331608603449639726"
	req.ActionId = "3331608605437739826"
	var actionOperators []*model.User
	actionOperator := model.User{}
	actionOperator.Name = "宋一"
	actionOperator.Contact = "10000000011"
	actionOperator.ContactType = "MOBILE"
	actionOperators = append(actionOperators, &actionOperator)
	req.ActionOperators = actionOperators
	response, err := sdkClient.Service(req)
	if err != nil {
		fmt.Println("request failed,", err.Error())
		return
...
func TestContractResend(t *testing.T) {
	req := request.ContractResendRequest{}
	reSendContract := model.ResendContract{}
	reSendContract.ContractId = "3331608603449639726"
	req.ResendContract = &reSendContract
	response, err := sdkClient.Service(req)
	if err != nil {
		fmt.Println("request failed,", err.Error())
		return
...
func TestAttachmentDownload(t *testing.T) {
	req := request.AttachmentDownloadRequest{}
	req.ContractId = "3331886389380793289"
	origin := false
	req.Origin = &origin
	file, _ := os.OpenFile("/Users/sgf/develop/临时/go/goAttchment.zip", os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0666)
	defer file.Close()
	err := sdkClient.Download(req, file)
	if err != nil {
		fmt.Println("error,", err.Error())
